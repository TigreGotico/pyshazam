...
The CLI is located in `apps/shazampy-cli/main.py` and wraps the `shazampy` library for quick shell usage.
...
```bash
python -m shazampy_cli identify --file <path>
```
...
```bash
python -m shazampy_cli scrape --artist-id <id>
```
...
The CLI is intentionally minimal. To add a `--json` flag or `--verbose` mode, modify `apps/shazampy-cli/main.py` and pass the flag into the `identify()` / `scrape()` coroutines.
